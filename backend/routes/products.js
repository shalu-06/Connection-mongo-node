...
router.get('/', (req, res, next) => {
  ...
  MongoClient.connect('mongodb+srv://max:xHZaAA8PPuVfxBS@cluster0.dqc6m.mongodb.net/shop?retryWrites=true&w=majority')
  .then(client => {
    const products = []
    client.db()
    .collection('products')
  ...
  .catch(err => {
    console.log(err);
    es.status(500).json({ message: 'An error occurred' });
  });
